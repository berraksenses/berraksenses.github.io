...
class Doggo {
    ...
    constructor() {
        ...
        this.dogGroup.position.set(1, 0.85, -4);
    ...
    stopWalking() {
        ...
        this.state = DOGGO_INITIAL_STATE;
        this.update();
    }

    moveTo(x, z, direction) {
...
function createDoggo() {
    ...
    const dogGroup = new THREE.Object3D();
    ...
    const leftDogLeg = createArm(dogLegFlag);
    const leftDogArm = createArm(dogLegFlag);

    const rightDogLeg = createArm(dogLegFlag);
    const rightDogArm = createArm(dogLegFlag);
    ...
    return {
        ...
        animDoggoWalking_1: new TWEEN.Tween(this.state).to(DOGGO_WALKING_1, 500).onUpdate(() => this.update()),
        animDoggoWalking_2: new TWEEN.Tween(this.state).to(DOGGO_WALKING_2, 500).repeat(Infinity).yoyo(true).onUpdate(() => this.update()),
